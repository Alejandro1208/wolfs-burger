...
function handleGetCategories() {
    ...
    $result = $conn->query("SELECT id, name, description, image_url, requirements FROM categories ORDER BY id ASC");
    $categories = [];
    if ($result) {
...
function handleCreateOrUpdateCategory() {
    ...
    if ($id) { 
        $stmt = $conn->prepare("UPDATE categories SET name = ?, description = ?, image_url = ?, requirements = ? WHERE id = ?");
        $stmt->bind_param("sssi", $name, $description, $image_path_for_db, $requirements, $id);
    } else { 
        $stmt = $conn->prepare("INSERT INTO categories (name, description, image_url, requirements) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $name, $description, $image_path_for_db, $requirements);
    }
